...
submitButton.addEventListener("click", function() {
    // 2) Query object is extracted from UI using global document object
    let query = CampusExplorer.buildQuery();
    // 3) Query object is sent to the POST /query endpoint using global XMLHttpRequest object
    CampusExplorer.sendQuery(query).then((result) => {
        // 4) Result is rendered in the reference UI by calling CampusExplorer.renderResult with the response
        // from the endpoint as argument
