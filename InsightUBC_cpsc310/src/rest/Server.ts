...
export default class Server {
    ...
    // create an instance of insightFacade:
    ...
    private static putDataset(req: restify.Request, res: restify.Response, next: restify.Next) {
        ...
        Server.insightFacade.addDataset(id, Buffer.from(req.body).toString("base64"), kind)
            .then((resu) => {
                res.json(200, {result: resu});
            }).catch((err) => {
    ...
    private static postQuery(req: restify.Request, res: restify.Response, next: restify.Next) {
        Server.insightFacade.performQuery(JSON.parse(req.body)).then((resu) => {
            res.json(200, {result: resu});
        }).catch((err) => {
            res.json(400, {error: err.message});
        });
    ...
    private static getDatasets(req: restify.Request, res: restify.Response, next: restify.Next) {
        Server.insightFacade.listDatasets().then((array) => {
            res.json(200, {result: array});
        }).catch((err) => {
            res.json(400, {error: err.message});
        });
        return next();
